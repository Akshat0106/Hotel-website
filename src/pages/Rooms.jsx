...
}
{
  /* 
{isFormOpen && selectedRoom && (
  <div className="fixed inset-0 flex justify-center items-center bg-transparent z-50 mt-40">
    {/* Overlay to make background slightly dim */
}
{
  /* <div className="absolute inset-0 bg-black bg-opacity-90"></div>  */
  ...
  return (
    <div className=" bg-gray-300">
      <div className="max-w-7xl mx-auto px-6 py-12 text-center ">
        ...
        {/* <div className="flex justify-center">
          <div
            className={`gap-8 ${filteredRooms.length === 1
                ? "flex flex-col items-center" // Single card centered
                : "grid grid-cols-1 sm:grid-cols-2 justify-items-center" // Two cards per row on medium+
              }`}
          >
            {filteredRooms.map((room) => (
              <div
                key={room.id}
                className="bg-white shadow-lg rounded-lg p-4 w-full max-w-[400px] h-auto mx-auto"
              >
                <img
                  src={room.src}
                  alt={room.name}
